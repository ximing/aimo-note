...
6. **版本回溯**：任意笔记可恢复到历史 revision，恢复行为本身形成新 revision。
...
| S3 Blob Store | markdown blob、快照、灾备档案 |
...
users/{userId}/vaults/{vaultId}/
  blobs/sha256/ab/cd/{fullHash}.md
...
```json
{
  ...
  "data": {
    "blobHash": "sha256:bbb",
    "storageKey": "users/usr_123/vaults/vlt_123/blobs/sha256/bb/bb/sha256_bbb.md",
...
```text
/api/v1/sync/history?vaultId=vlt_123&filePath=notes/today.md&page=1&pageSize=50
```
...
- 预签名 URL 仅允许上传到当前用户当前 vault 对应的 prefix
